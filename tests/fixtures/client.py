...
class FakeRepositoriesProvider(Provider):
    ...
    def __init__(self, repo: FakePaymentRepository, key_repo: FakeIdempotencyKeyRepository) -> None:
        super().__init__()
        self._repo = repo
...
async def make_client(
    ...
    container = make_async_container(
        FakeConfigProvider(),
        AuthProvider(),
        FakeRepositoriesProvider(fake_repo, fake_key_repo),
...
async def client(
    ...
    async with make_client(
        fake_repo, fake_key_repo, fake_session, ManualFakePaymentProvider()
    ) as ac:
        yield ac
